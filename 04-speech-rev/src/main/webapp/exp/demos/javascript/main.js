...
(function audioInit() {
  ...
  function initWebsocket(audioPromise) {
    ...
    function closeWebsocket() {
      ...
      //analyser = null; //tommy
      if (!isFinal) client2ndPass(result);
    }
	...
	//textBox.value = result.alternatives_[0].transcript_; //tommy
	//textBox.value = "";
	//for (var i=0;i<result.alternatives_.length-1;i++)
	//	textBox.value += result.alternatives_[i].transcript_ + ",";
	//textBox.value += result.alternatives_[result.alternatives_.length-1].transcript_;
      ...
      if (result.isFinal_) {
        isFinal = true;
	console.log("isFinal=true, apply 2nd pass adaptation.");
      	if (single_utterance_value) {
        	playButton.dispatchEvent(new Event('pause'));
		//console.log("single_utterace_value="+single_utterance_value);
		//console.log("isFinal=true, pause audio recording.");
	}
	//textBox.value = result.alternatives_[0].transcript_; //tommy
	client2ndPass(result);
/*	textBox.value = "";
	for (var i=0;i<result.alternatives_.length-1;i++)
		textBox.value += result.alternatives_[i].transcript_ + ",";
	textBox.value += result.alternatives_[result.alternatives_.length-1].transcript_;
