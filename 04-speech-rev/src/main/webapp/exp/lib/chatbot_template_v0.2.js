...
function chat(event) {
	...
	const url=dm_url.value+'?v=20170712&query='+document.getElementById('chat2_text1').value+'&lang=zh-tw&sessionId='+sessionID+'&timezone=Asia/Taipei';
	clientAccessToken = diagflow_token.value;
	Http.open("GET", url);
	//console.log("send RESTful request\n"+url);
	dm_log.innerText = "=> " + url;
...
function jarvisVoiceSearch3() {
	dms_url_new = 'https://dms.deltaww.com/dmsjarvisweb/zh-tw/SmartSearch?isTypeListExpand=1&typeId='+query_type+'&keywords=' + query_keyword;
